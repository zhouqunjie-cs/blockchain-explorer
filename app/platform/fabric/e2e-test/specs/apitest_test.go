...
func basicCheck(loginId string) {
	...
	It("get network list", func() {
		...
		resp := restGet("/authLedger/networklist", &NetworklistInfo{})
	...
	It("login to org1-network", func() {
		...
		resp := restPost("/authLedger/login", map[string]interface{}{"user": loginId, "password": "exploreradminpw", "network": "org1-network"}, &LoginResponse{})
	...
	It("fail to login with invalid credential", func() {
		...
		resp := restPost("/authLedger/login", map[string]interface{}{"user": loginId, "password": "invalid", "network": "org1-network"}, &LoginResponse{})
		result := resp.Result().(*LoginResponse)
	...
	It("get channels", func() {
		resp := restGetWithToken("/apiLedger/channels", &ChannelsResponse{}, token)
	...
	It("get channels info", func() {
		resp := restGetWithToken("/apiLedger/channels/info", &ChannelsInfoResp{}, token)
	...
	It("get block info", func() {
		resp := restGetWithToken("/apiLedger/block/"+channelGenesisHash+"/"+strconv.Itoa(blockHeight), &BlockResp{}, token)
		result := resp.Result().(*BlockResp)
		Expect(result.Status).Should(Equal(200))
	})

	It("get status of peers within commonchannel", func() {
		resp := restGetWithToken("/apiLedger/peersStatus/"+"commonchannel", &PeersStatusResp{}, token)
		result := resp.Result().(*PeersStatusResp)
		Expect(result.Status).Should(Equal(200))
	})

	It("get block activity", func() {
		resp := restGetWithToken("/apiLedger/blockActivity/"+channelGenesisHash, &BlockActivityResp{}, token)
...
var _ = Describe("REST API Test Suite - Single profile", func() {
	...
	Describe("Running REST API Test Suite in fabric-test", func() {
		...
		It("register user", func() {
			resp := restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "test", "password": "test", "network": "org1-network"}, &RegisterResp{}, token)
			resultRegister := resp.Result().(*RegisterResp)
			Expect(resultRegister.Status).Should(Equal(200))
		})

		It("list user", func() {
			resp := restGetWithToken("/apiLedger/userlist", &UserListResp{}, token)
			resultRegister := resp.Result().(*UserListResp)
			Expect(resultRegister.Status).Should(Equal(200))
		})

		It("unregister user", func() {
			resp := restPostWithToken("/apiLedger/unregister", map[string]interface{}{"user": "test", "network": "org1-network"}, &RegisterResp{}, token)
			resultRegister := resp.Result().(*RegisterResp)
			Expect(resultRegister.Status).Should(Equal(200))
		})
		...
		It("login with newly registered user", func() {
			resp := restPost("/authLedger/login", map[string]interface{}{"user": "test", "password": "test", "network": "org1-network"}, &LoginResponse{})
		...
		It("succeed to login with invalid credential", func() {
			...
			resp := restPost("/authLedger/login", map[string]interface{}{"user": "invalid", "password": "invalid", "network": "org1-network"}, &LoginResponse{})
			result := resp.Result().(*LoginResponse)
...
var _ = Describe("REST API Test Suite - Multiple profile", func() {
	...
	Describe("Running REST API Test Suite in fabric-test", func() {
		...
		Context("/authLedger/networklist", func() {
			It("get network list", func() {
				resp := restGet("/authLedger/networklist", &NetworklistInfo{})
				result := resp.Result().(*NetworklistInfo)
				nameList := []string{}
		...
		Context("/authLedger/login", func() {
			It("login to org1-network", func() {
				resp := restPost("/authLedger/login", map[string]interface{}{"user": "org1exploreradmin", "password": "exploreradminpw", "network": "org1-network"}, &LoginResponse{})
				result := resp.Result().(*LoginResponse)
				Expect(result.User.Message).Should(Equal("logged in"))
			...
			It("login to org2-network", func() {
				resp := restPost("/authLedger/login", map[string]interface{}{"user": "org2exploreradmin", "password": "exploreradminpw", "network": "org2-network"}, &LoginResponse{})
				result := resp.Result().(*LoginResponse)
				Expect(result.User.Message).Should(Equal("logged in"))
		...
		Context("/apiLedger/channels", func() {
		...
		Context("/apiLedger/channels/info", func() {
			...
			It("get channels info for org1", func() {
				...
				resp2 := restGetWithToken("/apiLedger/channels/info", &ChannelsInfoResp{}, token)
				...
				resp3 := restGetWithToken("/apiLedger/channels/info", &ChannelsInfoResp{}, token)
				...
				// resp4 := restGetWithToken("/apiLedger/channels/info", &ChannelsInfoResp{}, token)
				...
				resp5 := restGetWithToken("/apiLedger/channels/info", &ChannelsInfoResp{}, token)
		...
		Context("/apiLedger/block/(channelHash)/(blockHeight)", func() {
			...
			It("get block info for org1", func() {
				...
				resp3 := restGetWithToken("/apiLedger/block/"+channelGenesisHash+"/"+strconv.Itoa(latestBlockNum), &BlockResp{}, token)
		...
		Context("/apiLedger/register", func() {
			It("should work successfully with root admin user of org1", func() {
				token := restLogin("org1exploreradmin", "exploreradminpw", "org1-network")

				resp := restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testadmin1", "password": "testadmin", "roles": "admin"}, &RegisterResp{}, token)
				resultRegister := resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))

				resp = restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testadmin2", "password": "testadmin", "roles": "admin"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))

				resp = restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testuser1", "password": "testuser", "roles": "user"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))

				resp = restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testuser2", "password": "testuser", "roles": "user"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))
			})
			...
			It("should work successfully with newly added admin user of org1", func() {
				...
				resp := restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testadmin3", "password": "testadmin", "roles": "admin"}, &RegisterResp{}, token)
				resultRegister := resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))

				resp = restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testuser3", "password": "testuser", "roles": "user"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))
			})
			...
			It("should fail with newly added user of org1", func() {
				...
				resp := restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testadmin4", "password": "testadmin", "roles": "admin"}, &RegisterResp{}, token)
				resultRegister := resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(400))
				Expect(resultRegister.Message).Should(Equal("Error: Permission error : can't register user"))

				resp = restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "testuser4", "password": "testuser", "roles": "user"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(400))
				Expect(resultRegister.Message).Should(Equal("Error: Permission error : can't register user"))
			})
		...
		Context("/apiLedger/unregister", func() {
			It("should work successfully with root admin user of org1", func() {
				token := restLogin("org1exploreradmin", "exploreradminpw", "org1-network")
				...
				resp = restPostWithToken("/apiLedger/unregister", map[string]interface{}{"user": "testadmin1"}, &RegisterResp{}, token)
				resultRegister := resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))

				resp = restPostWithToken("/apiLedger/unregister", map[string]interface{}{"user": "testuser1"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(200))
			})
			...
			It("should fail to unregister with user of org1", func() {
				...
				resp = restPostWithToken("/apiLedger/unregister", map[string]interface{}{"user": "testuser2"}, &RegisterResp{}, token)
				resultRegister := resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(400))
			...
			It("should fail to unregister root admin or user which is logging in now", func() {
				...
				resp = restPostWithToken("/apiLedger/unregister", map[string]interface{}{"user": "org1exploreradmin"}, &RegisterResp{}, token)
				resultRegister = resp.Result().(*RegisterResp)
				Expect(resultRegister.Status).Should(Equal(400))
		...
		Context("/apiLedger/userlist", func() {
			It("should work successfully with root admin user of org1", func() {
				token := restLogin("org1exploreradmin", "exploreradminpw", "org1-network")
		...
		// 	resp := restGetWithToken("/apiLedger/peersStatus/"+"org2channel", &PeersStatusResp{}, token)
		// 	result := resp.Result().(*PeersStatusResp)
		// 	Expect(result.Status).Should(Equal(200))
		// })
		...
		// 	resp := restPostWithToken("/apiLedger/register", map[string]interface{}{"user": "test", "password": "test", "affiliation": "department2", "role": "admin"}, &RegisterResp{}, token)
		// 	resultRegister := resp.Result().(*RegisterResp)
		// 	Expect(resultRegister.Status).Should(Equal(200))
		// })

		// XIt("login with newly registered user", func() {
		// 	resp := restPost("/authLedger/login", map[string]interface{}{"user": "test", "password": "test", "network": "org2-network"}, &LoginResponse{})
		// 	resultLogin := resp.Result().(*LoginResponse)
