...
const login = ({ user, password }, networkObj) => dispatch =>
	post('/authLedger/login', { user, password, network: networkObj })
		.then(resp => {
...
const unregister = user => dispatch =>
	post('/apiLedger/unregister', { ...user })
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
