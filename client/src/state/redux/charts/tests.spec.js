...
describe('Charts', () => {
	describe('Operations', () => {
		...
		test('blockPerHour', () => {
			nock(/\w*(\W)/g)
				.get(`/apiLedger/blocksByHour/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('blockPerMin', () => {
			nock(/\w*(\W)/g)
				.get(`/apiLedger/blocksByMinute/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('changeChannel', () => {
			nock(/\w*(\W)/g)
				.get(`/apiLedger/changeChannel/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('transactionPerHour', () => {
			nock(/\w*(\W)/g)
				.get(`/apiLedger/txByHour/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('transactionPerMin', () => {
			nock(/\w*(\W)/g)
				.get(`/apiLedger/txByMinute/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('blockActivity', () => {
			nock(/\w*(\W)/g)
				.get(`/apiLedger/blockActivity/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
